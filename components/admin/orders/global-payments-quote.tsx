...
import { Globe, Send, Loader2, ShieldCheck } from 'lucide-react'
...
export function GlobalPaymentsQuote({ order }: QuoteProps) {
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          request_id: order.id,
          action: 'quote_sent',
          description: `Global Payment Quote sent for $${total.toFixed(2)}`,
          is_internal: false
        })
  ...
  // Handle Compliance Verify action
    ...
    try {
      ...
      await supabase
        ...
        .insert({
          ...
          description: `Compliance checks cleared for beneficiary ${order.metadata?.recipient_name}.`,
          is_internal: true
        })
  ...
  if (!['draft', 'quote_requested', 'compliance_check', 'quote_sent'].includes(order.status_code)) {
    return null
  }

  return (
    <div className="brutal-card bg-[#DCFCE7] p-6 mb-8 border-4 border-black shadow-[8px_8px_0px_var(--color-foreground)]">
      <div className="flex items-center justify-between mb-6 pb-4 border-b-4 border-black">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-black uppercase tracking-tighter">Compliance & Quote</h3>
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-80">Global Payments Module</p>
          </div>
        </div>
        ...
        {order.status_code === 'quote_requested' && (
          ...
          >
            Clear Compliance <ShieldCheck className="w-4 h-4" />
          </button>
        )}
      </div>
      ...
      <form onSubmit={handleSubmitQuote} className="space-y-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest opacity-80">Transfer Principal</label>
            <div className="flex">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest opacity-80">Taxes / Regulatory Fee</label>
            <div className="flex">
              <div className="bg-black text-white p-3 border-y-2 border-l-2 border-black font-black">$</div>
              <input 
                type="number"
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest opacity-80">Converto FX/Service Fee</label>
            <div className="flex">
              <div className="bg-black text-white p-3 border-y-2 border-l-2 border-black font-black">$</div>
              <input 
                type="number"
